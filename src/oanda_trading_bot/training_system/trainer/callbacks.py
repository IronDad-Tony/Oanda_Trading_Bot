...
try:
    ...
    if not _import_logged:
        logger.debug("callbacks.py (V5): Successfully imported logger from oanda_trading_bot.training_system.common.logger_setup.")
except ImportError:
    ...
    if not _import_logged:
        logger.warning("callbacks.py (V5): Failed to import logger from oanda_trading_bot.training_system.common.logger_setup. Using fallback logger.")

_config_cb_v5: Dict[str, Any] = {}
try:
    from oanda_trading_bot.training_system.common.config import LOGS_DIR as _LOGS_DIR, ACCOUNT_CURRENCY as _ACCOUNT_CURRENCY, INITIAL_CAPITAL as _INITIAL_CAPITAL
...
try:
    ...
    try:
        from oanda_trading_bot.training_system.trainer.stability_utils import NumericalStabilityMonitor
    except ImportError:
