    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        ...
        for slot_idx in self.current_episode_tradable_slot_indices:
            ...
            if units_to_trade.copy_sign(Decimal('1')) == projected_new_units.copy_sign(Decimal('1')) or abs(current_units) < Decimal('1e-9'): # 開倉或加倉
                if projected_total_margin_used_ac > self.equity_ac * (Decimal('1.0') - Decimal(str(OANDA_MARGIN_CLOSEOUT_LEVEL))): # 簡單檢查，避免立即觸發追繳
                    ...
                    # 計算最大可交易單位
                    max_affordable_margin_ac = self.equity_ac * (Decimal('1.0') - Decimal(str(OANDA_MARGIN_CLOSEOUT_LEVEL)))
