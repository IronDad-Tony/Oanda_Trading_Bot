...
class TradingLogic:
    ...
    def __init__(
        ...
        self.instruments = self.system_state.get_selected_instruments()
        self.granularity = self.config.get("trading_granularity", "S5")
        self.lookback_window = self.config.get("model_lookback_window", 100)

        # Initialize a data buffer for each instrument
        self.data_buffers: Dict[str, collections.deque] = {
            instrument: collections.deque(maxlen=self.lookback_window)
            for instrument in self.instruments
        }
        self.logger.info(f"TradingLogic initialized for instruments: {self.instruments}")
        self._initialize_all_data_buffers()
    ...
    def execute_trade_cycle(self):
        """
        Executes a single cycle of the trading logic for all configured instruments.
        """
        active_instruments = self.system_state.get_selected_instruments()
        if not active_instruments:
            self.logger.warning("No instruments selected for trading. Skipping logic cycle.")
            return
        ...
        for instrument in active_instruments:
            ...
            try:
                ...
                if new_candle['time'] > last_buffered_time:
                    ...
                    continue # No new data, skip to next instrument
